...
### DLC 지역명 한글 번역 (나무위키·Fandom 한글 위키 교차 검증)
- 키타카미(벽록의 가면) 11개: 북신 가도, 애플 힐스, 도깨비산, 등꽃 들판, 낙원의 황무지, 북신 원생지역, 영겁의 숲, 북신센터, 북신 합류지, 지옥골, 세벗 플라자
- 블루베리 아카데미(남청의 원반) 테라리움 4개 구역: 사바나, 코스트, 캐니언, 폴라
- 참고: DLC 정식 명칭은 "짙은보랏빛의 가면"이 아니라 **"벽록의 가면"**이었음(이전 기록 오류 정정)
- 남은 미번역: Reveler's Road, Crystal Pool, Torchlit Labyrinth, Chargestone Cavern 등 소수 지역(교차 검증 실패, 계속 영문 유지 — TODO 2번 참고)
...
2. **DLC 세부 지역명 한글 번역 — 대부분 완료 (2026-09-02 반영)**. Reveler's Road, Crystal Pool, Torchlit Labyrinth, Chargestone Cavern, Chilling Waterhead, Fellhorn Gorge, League Club Room, Oni's Maw, Area Zero Underdepths 등 소수 지역만 교차 검증 실패로 영문 유지 (완료된 15개는 위 "완료된 것" 참고).
